    ...
    for xml_file in glob.glob(path + '/*.xml'):
        ...
        for member in root.findall('object'):
            if not flag['face'] and (member[0].text=="face" or member[0].text=="face_es"):
                ...
                # Set file name
                row['file'] = root.find('filename').text
                # Reset flags
                flag['face'] = False
                flag['person'] = False
                ...
                # Save row value
                finalRow = (row['height'], row['face_x_min'], row['face_y_min'], row['face_x_max'], row['face_y_max'], row['person_x_min'], row['person_y_min'], row['person_x_max'], row['person_y_max'], row['class'], row['file'])
